...
export class StreamPlayback {
 private context?:AudioContext;private abort?:AbortController;private sources=new Set<AudioBufferSourceNode>();private generation=0;
 ...
 async play(input:SpeechInput){
  ...
  try{
   ...
   const schedule=(chunk:Uint8Array)=>{
    ...
    pending=pending.slice(samples*2);const source=context.createBufferSource();source.buffer=buffer;source.playbackRate.value=input.speaker==='旁白'?1.15:1;source.connect(context.destination);this.sources.add(source);
    ...
    nextTime=Math.max(nextTime,context.currentTime+.02);source.start(nextTime);nextTime+=buffer.duration/source.playbackRate.value;received=true;this.update('playing');
   };
   ...
   complete=true;if(!this.sources.size)this.update('ready');
