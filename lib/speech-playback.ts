...
export class SpeechPlayback {
  ...
  rate(value:number){if(this.clip&&this.clip.playbackRate!==undefined)this.clip.playbackRate=value;}
  ...
  async play(){
    ...
    try{
      ...
      if(clip.playbackRate!==undefined){let rate=1;try{rate=Number(localStorage.getItem('vn-speech-rate'))||1;}catch{}clip.playbackRate=Math.min(2.5,Math.max(.5,rate));}
